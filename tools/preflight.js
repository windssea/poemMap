...
/* ---------- 5. 强调色只有一套 ---------- */
/* 朱砂（选中/品牌印）与石绿石青（主题）各自成族，不该再有第三套饱和色。
   ⚠️ 这里的色值跟着《千里江山图》色板走，换色板时要同步——
   忘了同步的话这条会「因为找不到旧值而报失败」，看起来像回归，其实是检查本身过期。 */
const accentFams = {
  "朱砂": [/#a94f3c|#8f4030|rgba\(169, 79, 60/g],
  "石绿石青": [/#376b60|#2f5a51|#568c78|#376b76|#487565|rgba\(86, 140, 120/g],
};
const counts = {};
Object.keys(accentFams).forEach(function (k) {
  counts[k] = accentFams[k].reduce(function (s, re) { return s + (css.match(re) || []).length; }, 0);
});
check("强调色只有一套（朱砂 = 选中/品牌印；石绿石青 = 主题）",
  counts["朱砂"] > 0 && counts["石绿石青"] > 0,
  "朱砂 " + counts["朱砂"] + " 处 / 石绿石青 " + counts["石绿石青"] + " 处");
