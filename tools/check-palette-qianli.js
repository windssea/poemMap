...
const P = {
  juan:   "#f3efdf",  // 绢本底色
  warm:   "#faf7ed",  // 暖绢白
  moQing: "#263a35",  // 墨青 ← 2026-09-22 压深一档（9.75:1 → 10.48:1）
  body:   "#344840",  // 诗词正文
  ink2:   "#34463f",  // 次级 ← 压深一档（7.33:1 → 8.70:1）
  ink3:   "#59685f",  // 辅助 ← 压深一档（4.53:1 → 5.10:1）；方案 #747F74 只有 3.62:1
  ...
  zhuSha: "#a94f3c",  // 朱砂
  zhuShaD:"#8f4030",  // 深朱砂（数量角标用；浅朱砂 @.85 只有 3.18:1）
  land:   "#eadcb5",  // 陆地绢黄
  bian:   "#c6b78f",  // 边界赭灰
...
const CASES = [
  ...
  /* 数量角标：原为 #a94f3c @.85（压深合成后只剩 3.18:1，**不合格**）。
     2026-09-22 改成实色 var(--seal-d)。
     ⚠️ 方案给的 #9D4938 压在陆地绢黄上实算是 4.47:1，差 0.03 不到线，
     所以没有采纳；#8f4030 是 5.23:1。
     ⚠️ **已知取舍**：地标落在青绿山体上时只有 3.2:1（浅石绿上实测），
     任何还叫「朱砂」的颜色都到不了 4.5:1（#7a3020 才够，那已是黑褐）。
     撑住它的是地图小字那圈纸色硬描边，与山名、省名同一套机制。
     这一条**不计入下面的阈值**，因为按地形取值不是一个定值。 */
  ["text", "数量角标（陆地绢黄上）", P.zhuShaD, P.land],
