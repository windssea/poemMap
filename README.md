...
| 右上搜索框（末端山形标记 + 「/」快捷键提示）（**窄屏隐藏**） | `#tools .search`、`.search-mark`、`.q-kbd` |
| 筛选条（全部/先唐/唐诗/宋词 ｜ 全部/诗/词）+ 更多菜单（**窄屏隐藏**） | `.chipbar`、`#chipsDynasty`、`#chipsForm`、`#menuBtn` |
| 左侧篇目栏（**默认收起**，由左下 dock 唤出；页签右端带「N 首」实时计数、窄屏带关闭钮） | `#sidebar`、`#tabs`、`.tabs-n`、`#sideClose`、`#list` |
| 篇目行：篇名 / 唐·李白 / 📍地名 / 箭头 | `#list .item` |
| 左下统计 dock：篇目开关 + ⌘（窄屏）+ **当前筛选下**的 古诗/唐诗/宋词 首数 + N 首/N 地 + 换一批 | `#bottomBar`、`#sideToggle`、`#dockPalette`、`#shuffleBtn` |
...
390×844 上，「题名（33px）+ 搜索框与筛选条（87px）」要吃掉 145px，差不多六分之一屏，
而手机上真正要看的是地图。所以窄屏做了这几件事：

| 做法 | 理由 |
| --- | --- |
| `#brand`、`#tools` 整块 `display:none` | 题名是纯装饰；搜索与筛选搬进命令面板 |
| 底栏加一枚 `#dockPalette`（⌘） | 顶部收掉之后，搜索与筛选只剩这一个入口 |
| 命令面板补上朝代 / 体裁 / 索引 / 关于等动作 | 藏起顶栏不能等于砍掉筛选能力——`Palette.jsx` 的 `ACTIONS` 里都有 |
| 篇目栏加 `#sideClose` | 窄屏篇目栏几乎盖满屏，左下那颗「篇目」开关被压在下面，**原来关不掉** |
| 点篇目栏以外的地图也收起它 | 露出来的那条地图就是「点空白收起」的手势区（桌面不做，误触代价大） |
| `fitChina()` 窄屏上下留白收到 56 / 78 | 顶部已经没有东西了，这一块还给画面 |
| `env(safe-area-inset-*)` | `viewport-fit=cover` 下，home indicator 会压住底栏与缩放钮 |

> 顶部的搜索框在桌面端仍然在——只有窄屏把它换成了 `⌘`。
