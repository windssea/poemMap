...
│   ├── color-inventory.js  统计样式表里所有颜色值（改造前 234 种，改后 197 种）
...
这一版做的最重要的一件事是**把颜色收成一套语义色板**。
改造前 `style.css` 里有 **234 种颜色、374 次出现**，光「暖金/米色」一族就有 135 种：
同一个语义（分割线、控件描边、主题点）各自用了十几档不同的值。
页面看起来「糊」不是分辨率问题，是没有色板。现在收敛到 197 种，且剩下的都是
色板里的角色或其透明度变体。

| 角色 | 值 | 用在哪 | 对底对比度 |
| --- | --- | --- | --- |
| 页面底 · 宣纸白 | `#F6F3EA` | 大面积背景 | — |
| 内容表面 · 暖白 | `#FFFCF5` | 卡片、抽屉、弹窗 | — |
| 主文字 · 墨黑 | `#292E2B` | 标题、诗句 | 12.45:1 |
| 次级文字 | `#4E5A52` | 正文 | 7.0:1 |
| 辅助文字 · 灰绿 | `#6A6E68` | 作者、朝代、说明 | 4.68:1 |
| **主题色 · 山青** | `#4C675B` | 导航、筛选、链接、焦点环、**地图地标** | 5.57:1 |
| **强调色 · 朱砂** | `#A84F3F` | **只用于「当前选中」与品牌朱印** | 4.91:1 |
| 分割线 · 纸灰 | `#D8D0C3` | 描边、分隔 | 装饰用 |
| 控件描边 | `#8E8676` | 表单/按钮边界（要 3:1） | 3.25:1 |

**朱砂只留三处**：左上品牌朱印、右缘小印、以及**当前选中的那一个地标**。
顶栏筛选、底栏朝代、章节标题条原来全是红的，现在都归山青。
这样「红」在地图上有确切含义：这一处是你正在读的。

> ⚠️ **原方案的辅助文字 `#6D716B` 差 0.02 不达标。** 它在宣纸白上只有 **4.48:1**，
> 低于 AA 的 4.5:1（在更暖的卡面 `#FFFCF5` 上是 4.85:1，达标）。
> 所以压到 `#6A6E68`（4.68:1）——色相饱和度没动，只降了 0.2% 明度。
> 纸灰 `#D8D0C3` 对底只有 1.38:1，做**装饰分隔线**没问题，
> 但表单和按钮的边界另外给了 `#8E8676`（3.25:1），满足 WCAG 1.4.11。

**地标是四态而不是一种**，靠大小 + 明度 + 色相共同表达，不只靠颜色：

| 状态 | 表现 |
| --- | --- |
| 默认 | **墨青珠**（`--shan` 系渐变）+ 纸白外圈，低调但压在青绿山水与米黄省区上都读得出来 |
| 悬停 | `scale(1.28)` + 纸白圈加厚。**不加色**——鼠标一划过整片地图就红起来的话，选中态反而没了分量 |
| 选中 | 朱砂渐变 + 光晕 + 涟漪 |
...
它原来是 `#af4d38`，与「当前选中」的朱砂 `#a84f3f` 几乎同色——
...
用户反馈「放大缩小有卡顿感」。**第一件事是量，不是猜**——
写了 `tools/probes/zoom-perf.js`：做一次真实的缩放动画，逐帧记 rAF 间隔，
同时读引擎内部的调用计数。结果直接推翻了两个想当然的判断：

| 猜测 | 实测 |
| --- | --- |
| `layoutLabels()` 每帧都在跑，是元凶 | 缩放期间只跑 **1 次、1.3ms**。不是它 |
| 平移也卡 | 平移时它跑 45 次、共 18.4ms、单次 0.8ms。也不是它 |

真正的信号是 **zoomIn 里出现了一帧 347ms**。于是用
`tools/probes/zoom-ablate.js` 做**消融**——把图层按类摘掉再缩放：

| | p95 | max |
| --- | --- | --- |
| 基线（159 条矢量线 / 12361 点 + 215 地标） | 32.9ms | 305.8ms |
| 摘掉矢量层 | 21.6ms | 39.1ms |
| 摘掉地标 | **7.8ms** | 112.7ms |

**两个独立的成本**：尖峰来自矢量重投影，持续偏高来自地标。

#### 改了什么

| 改动 | 理由 |
| --- | --- |
| `markerZoomAnimation: false` | Leaflet 默认在缩放动画里逐个动每个 marker，215 个地标逐帧改 transform 把 p95 压在 30ms。关掉后地标跟着 pane 一起被 CSS 缩放，结束时归位——视觉几乎无差，每帧省下 200 多次写入 |
| 省区抽稀 0.05 → 0.12 度 | 省区只是晕染色块，不承担边界精度。国境视野下 0.12 度约 1.4px，看不出折线变直；点数 5740 → 3294 |
| 脊线按 tone 合并 | 原来 70 个 SVG 图元，按色调并成 4 条多段线，视觉完全一致（同色同粗细） |
| `smoothPath` seg 7 → 4 | 它是 Catmull-Rom 样条，seg 是**每段插几个点**，7 意味着点数乘七。线宽只有 1.1px，4 段与 7 段肉眼无异 |

⚠️ **不能**用 `zoomAnimation: false` 一刀切——那会连平滑缩放一起失去。

**结果**：连续 10 次缩放，**p95 稳定在 7.4–8.0ms**（原来 32.9 / 30.3），
动画期间稳定在 120fps 上下，离 60fps 的 16.7ms 预算还有一倍余量。
残余的是偶发单帧尖峰（19–148ms，10 次里 2 次过 100ms），来自 34 个省区路径的 SVG 重投影。

#### 没做的一件事，以及为什么

试过把省区的**径向晕染**换成平涂（省掉每省渐变的包围盒重算）。
`tools/probes/zoom-spike.js` 单看尾部数据像是从 127ms 掉到 32ms，
但看 `medianMax` 就发现**四个变体都在 36–41ms**——那个差距是尾部噪声，不是稳定效应。
而那个径向渐变是前人刻意的设计决定（「边缘再压深一档……才有『一块地』的厚度」），
为一次不可靠的收益把它换掉不划算，所以**还原了**。

要真正消灭尖峰只有两条路，都各有代价：把省区渐变也换成平涂（丢设计），
或整个矢量层改用 Canvas（Canvas 不支持 `fillColor: url(#渐变)`，
而 12361 个点里七成在渐变图层上）。

#### 诊断钩子

`window.__mk` 记 `layout` / `layoutMs` / `layoutMax` / `rove` 的调用次数与耗时；
`window.__layout` 暴露 `layoutLabels`；`window.__geoStats` 记抽稀前后的点数。
`tools/cdp.js` 支持 `CDP_VIEWPORT=宽x高`（任意中间宽度）与 `CDP_MOBILE`。
