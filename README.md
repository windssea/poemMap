...
收录 **301 首**（先唐 18 · 唐 143 · 宋 140），落在 **151 处**地标上。
苏轼 39 首、李白 35 首。
...
| 右侧**磨砂玻璃**详情抽屉（宽随诗长；竖排诗 + 注释 / 赏析 / 写作背景 / 作者 / **翻页** / **落款**） | `#detail`、`.d-col`、`#dPoem`、`.d-nav`、`.d-colophon` |
| 诗人 / 主题索引面板 | `#panel` |
...
- **地标不许被抽屉压住**：抽屉一开，`engine.revealPlace()` 检查该地标是否落在
  「没被抽屉挡住的那块画面」里；被挡住或贴边才 `flyTo` 挪一下。
  判据用 `detail.offsetWidth`（布局宽）而不是 `getBoundingClientRect()`——抽屉滑入时
  transform 还在动，rect.left 是动画中间值。
  ⚠️ 落点比例要给「占**未被遮住那块**的中点」，不能给「看着舒服」的下限：
  长诗抽屉能占到 94vw，剩下不到 200px，中点比例只有 0.07，
  给个 0.2 的下限反而把地标又算回抽屉底下。
...
| 项目 | 数量 |
| --- | --- |
| 诗词 | 301 首（先唐 18 · 唐 143 · 宋 140） |
| 体裁 | 诗 196 · 词 105 |
| 作者 | 111 位（`authors.js` 登记 113 条，2 条暂未用到） |
| 苏轼 39 首 · 李白 35 首 · 杜甫 13 首 | 三家占全库 29% |
| 地标 | 151 处（一处多诗已合并，最多越州永兴 16 首） |
| 标签 | 301 首全部标注（2–3 个主题词） |

> 「作者」两个数不一样是正常的：`AUTHOR_COUNT`（界面显示、抽屉落款用）
> 数的是**诗里真正出现过的**作者，`check-data.js` 报的是 `authors.js` 里
> **登记了几条**。留两条备用是有意的——先立传、后补诗。
