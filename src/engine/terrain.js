...
export const TERRAIN = (function () {
  ...
  /* ⚠️ 三元组的语义是 [山阴, 山腰, 山巅]，而渐变的画法是
     ...
     ⚠️ [2] 这一档原来是 #376b76 一带的「灰青绿」，压在山脊上与
     中石绿 #568c78 色相只差十几度，肉眼读成同一层——整座山于是
     是一片糊的青灰，看不出「山脊压石青」。
     现在按方案的《千里江山图》石青值收到 **#3e6b80 一带**：
     色相往蓝走约 8 度、饱和提一档，与石绿拉开可辨的距离，
     同时明度保持住，山脊不会变成一条蓝边。 */
  var TONES = [
    ["#94b5a0", "#568c78", "#3e6b80"],
    ["#9ab8a6", "#5c907d", "#426f84"],
    ["#8fb2a0", "#528a78", "#3a6680"],
    ["#98b6a3", "#5a8e7b", "#446f7e"],
    /* 4–7：**重点山体**专用的深石青档（方案 §3.1 的 #245968）。
       方案要的是"青绿局部重点 — 明亮绢本"的节奏：石青不是平均铺在山上的，
       而是几处大山压得更重、其余照旧。所以这里不去动上面四档，
       另起四档给 rank 1 且体量大的山系（天山、昆仑、横断、喜马拉雅、祁连…），
       由 RANGES 上的 `deep: true` 选用。
       面积由地形自己控制：深的只是这几条脊线那一线，山脚照样化进绢本。 */
    ["#94b5a0", "#4e8471", "#245968"],
    ["#9ab8a6", "#548878", "#2a5d6c"],
    ["#8fb2a0", "#4a8070", "#1f5464"],
    ["#98b6a3", "#528674", "#28596a"],
  ];
  ...
  var FAR_TONES = [
    ...
    /* 4–7 与 0–3 相同：远山是"雾里的山"，本来就与主山拉不开——
       重点山体压深的是**主山那条脊线**，远山仍旧一片浅青灰。
       数组长度必须与 TONES 对齐（rgf{t} 按同样的下标取色）。 */
    ["#c6d4c4", "#a8c0b0", "#8aa6a6"],
    ["#c9d6c7", "#adc3b4", "#8fa9aa"],
    ["#c4d3c5", "#a5bdb0", "#86a3a4"],
    ["#c7d5c6", "#abc2b2", "#8ca7a8"],
  ...
  /* 山系：name 名称（同时用于地图标注）；rank 1 主要 / 2 次要（标注优先级）；
     spine 脊线；w 宽度（纬度度数）；sp 山组间距；tone 色调；deep 是否为
     「重点山体」（用 4–7 那一档更深的石青，方案 §3.1）；label 可选的标注坐标覆盖。

     位置与大小都由 tools 里的校验脚本比对过（脊线落在哪个省、长度与宽度是否与
     真实山脉同一量级）：长条形的山要「长而窄」，孤山、小山不能画成一团胖云。
     校验方式见 tools/check-ranges.js。 */
  var RANGES = [
    { name: "天山", rank: 1, spine: [[43.9, 80.2], [43.3, 83.5], [43.0, 87.0], [42.4, 90.5], [41.8, 94.0]], w: 3.2, sp: 0.85, tone: 3, deep: true },
    { name: "阿尔泰山", rank: 2, spine: [[48.6, 85.6], [47.6, 88.5], [46.8, 91.2]], w: 2.3, sp: 0.9, tone: 3 },
    { name: "昆仑山", rank: 1, spine: [[36.4, 75.5], [35.8, 79.5], [35.3, 83.5], [35.6, 87.5], [35.2, 91.5], [36.0, 95.0]], w: 3.0, sp: 0.9, tone: 3, deep: true },
    { name: "喜马拉雅", rank: 1, spine: [[30.9, 79.8], [30.1, 83.0], [29.0, 86.0], [28.4, 89.0], [28.8, 92.0], [29.8, 94.8]], w: 2.4, sp: 0.9, tone: 1, deep: true },
    { name: "唐古拉山", rank: 2, spine: [[32.9, 90.0], [33.0, 92.4], [32.6, 94.4], [33.0, 96.4]], w: 1.4, sp: 1.0, tone: 2 },
    { name: "祁连山", rank: 1, spine: [[39.0, 95.8], [38.4, 98.5], [37.6, 101.5], [37.0, 103.6]], w: 2.0, sp: 0.85, tone: 3, deep: true },
    ...
    { name: "横断山", rank: 1, spine: [[31.5, 98.5], [29.5, 99.5], [27.5, 100.3], [25.8, 101.3], [24.2, 102.0]], w: 2.4, sp: 0.85, tone: 1, deep: true },
  ...
  function build() {
    ...
    for (var i = 0; i < RANGES.length; i++) {
      ...
      /* 重点山体（方案点名的那五条）换用 4–7 那一档更深的石青；
         其余山系照旧用 0–3。深色因此只落在几条大脊上，
         不会把整张图的山都拉深一档。 */
      var tone = r.deep ? r.tone + 4 : r.tone;
      /* 远山：同一条脊线，整体北抬、起伏略大，且用另一组种子错开峰谷。
         它只当「一排山尖」——露在主山脊线之上，其余被主山压住。
         北抬量不宜大：抬得越高，露出的浅色越多，越像另画了一排雪山。 */
      ridgeShape(sp, r.w * 1.08, tone, seed + 37.4, out,
        { far: true, shift: 0.14, botAbs: 0.20, amp: 0.78 });
      /* 主山：主要山脉才点亮脊，小山不点（否则整图到处是白线） */
      ridgeShape(sp, r.w, tone, seed, out,
